...
- Electricity Rate: $${
        location.electricityRate ?? "Not available"
      }/kWh
- Solar Score: ${location.solarScore ?? "Not available"}/10
- Note: ${location.note ?? "Not available"}
`;
    } else {
      contextSummary += `
Selected Location:
...
`;
    ...
    if (calculator) {
      contextSummary += `
Calculator Snapshot:
- Scenario: ${calculator.scenario ?? "Not available"}
- Rate Mode: ${calculator.rateMode ?? "Not available"}
- Manual Electricity Rate: $${
        calculator.manualElectricityRate ?? "Not available"
      }/kWh
- Active Electricity Rate: $${
        calculator.activeElectricityRate ?? "Not available"
      }/kWh
- Total Project Cost: $${calculator.totalProjectCost ?? "Not available"}
- Annual Revenue: $${calculator.annualRevenue ?? "Not available"}
- Net Operating Income: $${
        calculator.netOperatingIncome ?? "Not available"
      }
- Payback Period: ${
        typeof calculator.paybackPeriod === "number"
          ? `${calculator.paybackPeriod.toFixed(1)} years`
          : "Not achievable"
      }
- NPV: $${calculator.npv ?? "Not available"}
`;
    } else {
      contextSummary += `
Calculator Snapshot:
